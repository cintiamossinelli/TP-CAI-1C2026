...
public class ImposicionCDModelo
{
        ...
        foreach (var detalle in detallesAgregados)
        {
            ...
            for (int i = 0; i < detalle.Cantidad; i++)
            {
                ...
                guias.Add(new GuiaEntidad
                {
                    ...
                    TipoCaja = tipoCaja,
                    PrecioVenta = CalcularPrecioVenta(cuitCliente, tipoCaja, agenciaChecked, domicilioChecked),
                    Estado = estado,
    ...
    private static decimal CalcularPrecioVenta(
        string cuitCliente,
        TipoTamañoEnvioEnum tipoCaja,
        bool agenciaChecked,
        bool domicilioChecked)
    {
        decimal precioBase = ClienteAlmacen.Clientes
            .FirstOrDefault(cliente => cliente.CuitDniCuilCliente == cuitCliente)?
            .Tarifario
            .FirstOrDefault(precio => precio.TamañoEncomienda == tipoCaja)?
            .Importe ?? 0;

        decimal tarifaExtra = 0;
        if (agenciaChecked)
        {
            tarifaExtra = ObtenerTarifaExtra((TipoExtraEnum)3);
        }
        else if (domicilioChecked)
        {
            tarifaExtra = ObtenerTarifaExtra((TipoExtraEnum)2);
        }

        return precioBase + tarifaExtra;
    }
    ...
    private static List<HistorialGuia> CrearHistorial(EstadoGuiaEnum estado, DateTime fecha)
    {
        ...
        {
            new HistorialGuia { Fecha = fecha, Estado = EstadoGuiaEnum.ImpuestaEnCD },
            new HistorialGuia { Fecha = fecha, Estado = EstadoGuiaEnum.PendienteDeAdmision },
